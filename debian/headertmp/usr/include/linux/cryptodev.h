...
struct crypt_op {
	...
	__u8	*dst;	/* pointer to output data */
...
struct crypt_auth_op {
	...
	__u8	*auth_src;	/* authenticated-only data */

	/* The current implementation is more efficient if data are
	 * encrypted in-place (src==dst). */
	__u8	*src;	/* data to be encrypted and authenticated */
	__u8	*dst;	/* pointer to output data. Must have
				 * space for tag. For TLS this should be at least
				 * len + tag_size + block_size for padding
				 */

	__u8    *tag;    /* where the tag will be copied to. TLS mode
